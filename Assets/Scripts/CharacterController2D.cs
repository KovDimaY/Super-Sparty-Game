...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
        ...
        //Allow double jump if is grounded
        if (isGrounded)
        {
            canDoubleJump = true;
        }
        ...
        {
            DoJump();
            //disable double jump after two jumps
            canDoubleJump = false;
        }
    ...
    // coroutine to teleport the player
    IEnumerator Teleportation(float time, Vector3 newPosition)
    {
        if (playerCanMove)
        {
            // freeze the player
            ...
            // After waiting tell the GameManager to begin teleportation
            yield return new WaitForSeconds(time);

            if (GameManager.gm) // if the gameManager is available, tell it to teleport the player
    ...
    public void CollectExtraLife(int amount)
    {
        PlaySound(coinSFX);

        if (GameManager.gm) // add the lives through the game manager, if it is available
            GameManager.gm.AddLife(amount);
    }

    //save position of the player after touching a check point to respawn him here after death
    public void SavePosition()
    {
        PlaySound(coinSFX);

        if (GameManager.gm) // set a new spawn location of the player through the game manager, if it is available
            GameManager.gm.SetLocation(_transform.position);
    }

    //teleportation of the player after touching a teleport
    public void TeleportPlayer(float time, Vector3 newPosition)
    {
        PlaySound(coinSFX);
        StartCoroutine(Teleportation(time, newPosition));
    }
    ...
    //make player bounce after stunning enemies
