...
public class GameManager : MonoBehaviour {
    ...
    // set a new spawn location of the player
    public void SetLocation(Vector3 newSpawnLoc)
    {
        // set new location
        _spawnLocation = newSpawnLoc;

    }

    // public function to add lives and update the gui of the player prefs accordingly
    public void AddLife(int amount)
    {
        // increase lives if it is possible
        if (lives + amount <= UIExtraLives.Length+1)
        {
            lives += amount;
        } else
        {
            lives = UIExtraLives.Length+1;
        }
